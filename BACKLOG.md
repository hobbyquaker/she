...
- **per-file git history panel** — right-click a file → "Show git history" (only when `gitInfo !== null`); shows a commit list in the left aside below the file tree (fixed ~220 px, scrollable, "Load more" button). Clicking a commit opens a **separate** read-only diff overlay (not the AI diff overlay) showing historic content vs. current file content (open tab if available, otherwise fetched from disk). Closing the history panel closes the diff. Binary files: show "diff not available" message. Rename tracking: accept limitation — `--follow` is used in `git log` but `git show <hash>:<currentPath>` may fail for commits before a rename; keep in backlog for later. Requires two new backend routes: `GET /she/git/log?path=&limit=` (parses `git log --follow --format=...`) and `GET /she/git/show?hash=&path=` (returns `{ content, binary }`).
