...
Fields are *secret* or *plain*: for secret fields the HTTP API is write-only — names and change times are readable, the value is not, by any route; plain fields (a user name, a host) are listed in clear and are not redacted in logs. The kind is chosen at creation; a plain field can be marked secret (one-way), never the reverse — the only way back to readable is delete and re-create. With `--auth none` anyone who can reach she can *overwrite* secrets (as they can edit scripts, which can read them) — the trust model is the same as for scripts. Known secret values are redacted to `***` in the log sinks (Logs tab, `she.jsonl`, console) so a logged response body does not leak a token; that is a convenience, not a guarantee — a script can still publish a secret to MQTT or write it into sheDB.
