...
| Path | Purpose |
| --- | --- |
| `/etc/systemd/system/<adapter>@.service` | template unit (one per adapter; she recognises it by its `EnvironmentFile=/etc/<adapter>/%i.env` line). Units written by early core versions do not read the shared `broker.env`; the Adapters tab marks those *no broker.env* — reinstalling one instance of the adapter rewrites the unit |
