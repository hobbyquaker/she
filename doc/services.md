...
she-servicectl node   update [--lts|--latest]                download the pinned tj/n if needed, then n install lts (or latest); JSON: version before/after, what n installed, mismatch
...
**Node.js on a host.** The Hosts tab shows each host's node version next to the helper version. she asks nodejs.org once for all hosts (`GET /she/services/node/releases`, cached 6 h) which versions the two labels n understands currently resolve to — `lts`, the newest long-term-support release, and `latest`, the newest release of all — and offers one button per label the host does not already run, each carrying the version it would install. A host on both is simply *up to date*, and no button is shown. (n also accepts `stable`, but only as its own old alias for `lts`, so it is not a third channel: the helper maps it to `lts`.) A button runs `she-servicectl node update`, then `n install lts` (or `latest`) with `N_PREFIX` at `/usr/local`.
